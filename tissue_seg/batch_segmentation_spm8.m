function batch_segmentation_spm8(image_pool, t, options)
% ************************************************************************
% Batch image segmentation with SPM8 toolbox. The function is mainly
...
% 
%         --> number of threat (just to print and loosely control the 
%               processing flow
%
% options -->  some SPM8 options to set:
%  
%              SPM_PATH = 'folder where SPM8 lives'
%              BUILD_DISCRETE = 'if 1, an additional scan with the 3 main 
%                              tissues is also stored. 
%              PACKRESULTS = 'if 1, results are packed in a SPM8 folder in the location
...
% ************************************************************************
    ...
    for s=1:num_scans
        ...
        if BUILD_DISCRETE
            ...
            save_compressed_nii(csf_in,[seg_path,'/',scan_number,'_SPM8']);
        end
        ...
        if PACK_RESULTS
           mkdir([seg_path,'/SPM8']);
           movefile([seg_path,'/c1',identifier,'.nii.gz'], [seg_path,'/SPM8/']);
           movefile([seg_path,'/c2',identifier,'.nii.gz'], [seg_path,'/SPM8/']);
           movefile([seg_path,'/c3',identifier,'.nii.gz'], [seg_path,'/SPM8/']);
           movefile([seg_path,'/',scan_number,'_SPM8.nii.gz'], [seg_path,'/SPM8/']);
           if options.SPM_SAVE_TRANSFORMATIONS
               movefile([seg_path,'/y_',identifier,'.nii'], [seg_path,'/SPM8/']);
               movefile([seg_path,'/iy_',identifier,'.nii'], [seg_path,'/SPM8/']);
           end
        end
